  ...
  <?php foreach ($item as $i): ?>
    <?php if ($i->is_sold_out == 0): ?>
      <div class="row align-items-center menu-item position-relative">
          ...
          <!-- <?php if ($i->harga_weekday == 0): ?>
              ...
              <?php else: ?>
                <?php if ($promo_value != 0): ?>
                  <label class="price"><span style="text-decoration: line-through; color: red;">Rp <?= number_format(max($harga_asli, 0)) ?></span></label>
                <?php endif ?>
                  
                  <label class="price">Rp <?= number_format(max($harga_akhir, 0)) ?></label> <!-- Harga akhir setelah diskon -->
              <?php endif; ?>
        ...
        <div class="col-3 text-end position-relative">
            <?php 
            // Cek apakah item ini termasuk Special Promo
            $cekSPchart = $this->Item_model->cekSPchart($i->no); 
            $cekSPtrans = $this->Item_model->cekSPtrans($i->no); 

            // Cek apakah ada Special Promo lain di cart/trans (tanpa filter item_code)
            ...
            <?php if ($i->sub_category == 'Special Promo'): ?>
                ...
                $sudahAdaSP = ($adaSPchart || $adaSPtrans); // ada Special Promo lain
                ?>

                <?php if (!$sudahAdaSP || $itemIniAda): ?>
                    <!-- Belum ada SP sama sekali, atau ini item SP yang sama → BOLEH -->
                    <div class="add-btn">
                        <a href="<?= base_url() ?>index.php/ordermakanan/detailmenu/<?= $i->id ?>/<?= str_replace(" ","%20",$i->sub_category) ?>#<?= str_replace(" ","_",$i->sub_category) ?>" 
                           style="padding: 3px;color: white;font-size: 25px;">
                            <i class="bi bi-plus"></i>
                        </a>
                    </div>
                <?php else: ?>
                    <!-- Ada Special Promo lain → TIDAK BOLEH -->
                    <div class="add-btn-muted" style="opacity: 0.5; cursor: not-allowed;">
                        <i class="bi bi-plus" style="font-size: 25px;"></i>
                    </div>
                <?php endif; ?>

            <?php else: ?>
                <!-- Item non-Special Promo selalu boleh -->
                <div class="add-btn">
                    <a href="<?= base_url() ?>index.php/ordermakanan/detailmenu/<?= $i->id ?>/<?= str_replace(" ","%20",$i->sub_category) ?>#<?= str_replace(" ","_",$i->sub_category) ?>" 
                       style="padding: 3px;color: white;font-size: 25px;">
                        <i class="bi bi-plus"></i>
    ...
    <?php else: ?>
      <div class="row align-items-center menu-item position-relative">
        ...
        <div class="col-6">
          <h6 class="text-muted"><?= $i->description ?></h6>
          <!-- <?php if ($i->harga_weekday == 0): ?>
                <h6>Free</h6>
              <?php elseif ($i->harga_weekend == 0): ?>
                <h6>Free</h6>
              ...
              <?php else: ?>
                ...
                <?php if ( $holiday == NULL): ?>
                   <?php  if ($hr == "Saturday" || $hr == "Sunday") :?>
                    <p class="price text-muted">Rp <?= number_format($i->harga_weekend) ?></p>
                    <?php else: ?>
                    <p class="price text-muted">Rp <?= number_format($i->harga_weekday) ?></p>
                  <?php   endif ?>
                <?php  else: ?>
                  <?php  if ($hr == "Saturday" || $hr == "Sunday") :?>
                    <p class="price text-muted">Rp <?= number_format($i->harga_weekend) ?></p>
              ...
              <?php else: ?>
                <?php if ($promo_value != 0): ?>
                  <label class="price text-muted"><span style="text-decoration: line-through; color: red;">Rp <?= number_format(max($harga_asli, 0)) ?></span></label>
                <?php endif ?>
                  
                  <label class="price text-muted">Rp <?= number_format(max($harga_akhir, 0)) ?></label> <!-- Harga akhir setelah diskon -->
              <?php endif; ?>
